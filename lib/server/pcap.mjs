...
// `deviceIp` = IP dell'interfaccia attiva da catturare (default: prima interfaccia utile).
// `onFlow(flow)` opzionale: callback per DPI sul primo payload.
export function startCapture({ localIps = new Set(), deviceIp, onFlow } = {}) {
  ...
  try {
    ...
    const c = new Cap();
    // Preferisci l'interfaccia con l'IP locale attivo; fallback al device di default.
    const device = (deviceIp && Cap.findDevice(deviceIp)) || Cap.findDevice();
