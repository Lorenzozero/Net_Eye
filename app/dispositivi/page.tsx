...
export default function DispositiviPage() {
    ...
    // Top vendor: ignora i valori generici (MAC privato / sconosciuto)
    const generic = (v: string) => !v || v === 'Unknown' || v.startsWith('MAC privato');
    const vendorCounts = devices.reduce((acc, device) => {
        const vendor = device.vendor || 'Unknown';
        if (!generic(vendor)) acc[vendor] = (acc[vendor] || 0) + 1;
        return acc;
        ...
        <Layout>
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
                ...
                <div className="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg border border-slate-200/70 dark:border-white/10">
                    <div className="p-5">
                        <div className="flex items-center">
                            ...
                            <div className="ml-5 w-0 flex-1">
                                <dl>
                                    <dt className="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Online / Offline</dt>
                                    <dd>
                                        <div className="text-lg font-medium text-gray-900 dark:text-white">
                                            <span className="text-green-600 dark:text-green-400">{onlineCount}</span>
                ...
                <button
                    ...
                    className="text-left bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg border border-slate-200/70 dark:border-white/10 cursor-pointer hover:ring-2 hover:ring-orange-400 hover:-translate-y-0.5 transition-all">
                    <div className="p-5">
                        <div className="flex items-center">
                            <div className="flex-shrink-0">
                            ...
                            <div className="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd>
                                        <div className="text-lg font-medium text-gray-900 dark:text-white">{openPortsCount} <span className="text-xs font-normal text-indigo-500">· dettagli</span></div>
                                    </dd>
                                </dl>
                            </div>
                        </div>
            ...
            {portsOpen && <PortsModal devices={devices} onClose={() => setPortsOpen(false)} />}
