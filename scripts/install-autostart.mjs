...
function installWindows() {
  ...
  const vbs =
    ...
    `sh.Run "cmd /c node ""${serverPath}"" > ""${log}"" 2>&1", 0, False\r\n`;
...
function installMac() {
  ...
  writeFileSync(file,
    ...
    `  <key>ProgramArguments</key><array><string>node</string><string>${serverPath}</string></array>\n` +
...
const path = fn();
console.log(`✅ Autostart installato (livello utente, nessun permesso admin):\n   ${path}`);
console.log('   Lo scanner partirà automaticamente al prossimo login.');
